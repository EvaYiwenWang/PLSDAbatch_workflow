...
Scatter_Density <- function(object,
  ...
  pMain <- ggplot(data = data, aes(x = data[ ,1], y = data[ ,2], colour = batch, shape = trt)) +
    ...
    scale_color_manual(values = color.mixo(1:10)) + theme_bw() +
  ...
  # top density plot
  pTop <- ggplot(data = data, aes(x = data[ ,1], fill = batch, linetype = trt)) +
  ...
  # right density plot
  pRight <- ggplot(data = data, aes(x = data[ ,2], fill = batch, linetype = trt)) +
  ...
  if(is.null(batch) && is.null(trt)){legend <- grid.rect(gp = gpar(col="white"))}else{
...
#' @param x.vjust Numeric, vertical justification of x axis, in the range of \eqn{0} to \eqn{1}.
#'
#' @return none
#' @export
#'
...
box_plot <- function(df, title = NULL,
  ...
  value <- df[,1]
  batch <- df[,2]
  ggplot(data = df, aes(x = batch, y = value, fill = batch)) + stat_boxplot(geom = "errorbar", width = 0.4) +
    geom_boxplot() + scale_fill_manual(values = color.mixo(1:10)) + theme_bw() +
    theme(axis.text.x = element_text(angle = x.angle, hjust = x.hjust, vjust = x.vjust),
...
#' @param title.hjust Numeric, horizontal justification of the plot title, in the range of \eqn{0} to \eqn{1}.
#'
#' @return none
#' @export
#'
