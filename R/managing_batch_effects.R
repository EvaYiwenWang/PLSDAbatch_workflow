...
#' @param batch.fix A factor or a class vector for the batch grouping information
#' (categorical outcome variable), treated as a fixed effect in the model.
#' @param batch.fix2 A factor or a class vector for a second batch grouping information
#' (categorical outcome variable), treated as a fixed effect in the model.
#' @param batch.random A factor or a class vector for the batch grouping information
#' (categorical outcome variable), treated as a random effect in the model.
...
linear_regres <- function(data,
  ...
  if(type == 'linear model'){
    ...
    model <- list()
    p <- c()
    R2 = adj.R2 = RMSE = RSE = AIC = BIC = data.frame(trt.only = NA,
                                                      trt.batch = NA)
  ...
  if(type == 'linear mixed model'){
    ...
    model <- list()
    p <- c()
    cond.R2 = marg.R2 = RMSE = RSE = AIC = BIC = data.frame(trt.only = NA,
                                                            trt.batch = NA)
    for(i in seq_len(ncol(data))){
      res.lm0 <- lm(data[,i] ~ trt)
      ...
      if(!is.null(batch.fix)){
        if(!is.null(batch.fix2)){
          ...
          res.lmm <- lmer(data[,i] ~ trt + batch.fix + (1|batch.random))}
        }else{
      ...
      marg.R2[i, ] <- c(summary.res0$r.squared, as.numeric(r2(res.lmm)[2]))
