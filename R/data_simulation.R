...
#' @param noise Numeric, the variability (standard deviation) of background noise.
#' @param N Integer, the number of samples.
#' @param p_total Integer, the number of variables.
#' @param p_trt_relevant Integer, the number of variables affected with treatment effects.
#' @param p_bat_relevant Integer, the number of variables affected with batch effects.
...
#' @param percentage_overlap_variables Numeric, in the range of \eqn{0} to \eqn{1},
#' the percentage of variables with both batch and treatment effects given fixed number of variables with each effect.
#' @param seeds Integer. Default value is randomly selected from \eqn{0} to \eqn{999}.
#'
#'
...
#' \item{Y.trt}{The simulated outcome variable indicating the treatment grouping information.}
#' \item{Y.bat}{The simulated outcome variable indicating the batch grouping information.}
...
#'                        percentage_samples = 0.5, percentage_overlap_variables = 0.5)
#'
#' ## Second example
...
simData = function(bat.mean,
  ...
  tComp_relevant <- c(tCompG1[1:(n*percentage_samples)],
                      tCompG2[1:(n*(1-percentage_samples))],
  ...
  w.relevant.t <- sample(c(runif(p_trt_relevant/2,-0.3,-0.2),runif(p_trt_relevant/2,0.2,0.3)))
  w.relevant.t <- w.relevant.t/sqrt(sum(w.relevant.t^2))
  Dat.relevant.t <- matrix(tComp_relevant, ncol = 1) %*% matrix(w.relevant.t, nrow = 1)


  ### batch loadings
  ## p_bat_relevant variables with batch effects
  w.relevant.b <- sample(c(runif(p_bat_relevant/2,-0.3,-0.2),runif(p_bat_relevant/2,0.2,0.3)))
  w.relevant.b <- w.relevant.b/sqrt(sum(w.relevant.b^2))
  Dat.relevant.b <- matrix(bComp_relevant, ncol = 1) %*% matrix(w.relevant.b, nrow = 1)
  ...
  index.b <- c(sample(index.t, overlap.num*percentage_overlap_variables),
               sample((1:p_total)[-index.t], (p_bat_relevant-overlap.num*percentage_overlap_variables)))
  ...
  rownames(finalData) <- rownames(cleanData) <- paste0('sample', 1:N)


  ## outcome variables
  Y.bat <- rep(c("bat1", "bat2"), each = n)
  Y.trt <- c(rep("trt1", n*percentage_samples), rep("trt2", n*(1-percentage_samples)),
             rep("trt1", n*(1-percentage_samples)), rep("trt2", n*percentage_samples))
