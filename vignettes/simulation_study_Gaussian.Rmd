---
title: 'Simulation Study (Gaussian distribution)'
author: 
- 'Yiwen (Eva) Wang'
...
vignette: >
  %\VignetteIndexEntry{Simulation Study (Gaussian distribution)}
...
|    Parameters    | $\mu_{(trt)}$ | $\sigma'_{(trt)}$ | $\mu_{(batch)}$ | $\sigma'_{(batch)}$ |   $M^{(trt)}$   |  $M^{(batch)}$  | $M^{(trt \ \& \ batch)}$ |
|:----------------:|:-------------:|:-----------------:|:---------------:|:-------------------:|:---------------:|:---------------:|:------------------------:|
...
To save space, here we only describe one scenario that we believe is a representative of real data ($M^{(trt \ \& \ batch)} = 30$, simulation 6 in Table 1).
...
```{r, fig.cap = 'Figure 1: Simulation studies (Gaussian distribution): comparison of explained variance before and after batch effect correction for the balanced batch × treatment design.'}
# global variance (RDA)
