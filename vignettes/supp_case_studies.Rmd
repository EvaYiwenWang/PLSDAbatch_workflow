---
title: 'Supplementary Materials for PLSDA-batch Vignette'
author: 
- 'Yiwen (Eva) Wang'
...
In the above figure, the tissue variation (effects of interest) accounted for 24% of data variance on component 1, while the gel variation (batch effects) for 21% on component 2. Therefore, the batch effect was slightly weaker than the treatment effect in the `r colorize("sponge data", "brown")`.
...
In the result, `X1` and `X2` represent the first and second covariates fitted in the model. `[a]`, `[c]` represent the independent proportion of variance explained by `X1` and `X2` respectively, and `[b]` represents the intersection variance shared between `X1` and `X2`. The `r colorize("sponge data", "brown")` have a completely balanced batch x treatment design, so there was no intersection variance (indicated in line `[b]`, Adj.R.squared = -0.01) detected. The proportion of treatment and batch variance is nearly equal as indicated in lines `[a]` and `[c]`.
...
The methods that we use to account for batch effects include the method designed for microbiome data: zero-inflated Gaussian (ZIG) mixture model and the methods adapted for microbiome data: linear regression, SVA and RUV4. Among them, SVA and RUV4 were designed for unknown batch effects.

As the ZIG model is applicable to counts data, the `r colorize("sponge data", "brown")` are not qualified for this model. We therefore only apply the methods that were adapted for microbiome data.
...
To diagnose the validity of the model fitted with both treatment and batch effects, we use different plots to check the assumptions of each microbial variable. For example, the diagnostic plots of "OTU2" are shown in the above figure panel. The linearity (or homoscedasticity) and homogeneity of variance were not satisfied (top panel). The correlation between batch (`batch.fix`) and treatment (`trt`) effects was very low, indicating a good model with low collinearity. Some samples could be classified as outliers with a Cook's distance larger than or equal to 0.5, for example, "21", "15", "7", "1" and "13" (middle panel). The distribution of residuals was not close to normal (bottom panel). For the microbial variables with some assumptions not met, we should be careful about their results. 
...
The methods that we use to correct for batch effects include removeBatchEffect, ComBat, PLSDA-batch, sPLSDA-batch, Percentile Normalisation and RUVIII. Among them, RUVIII was designed for unknown batch effects. Except RUVIII that requires sample replicates which the `r colorize("sponge data", "brown")` do not have, these methods were all applied to and illustrated with the `r colorize("sponge data", "brown")`.
...
```{r Spongepca, fig.height = 17, fig.width = 12, out.width = '100%', echo = F, fig.align = 'center', fig.cap = 'The PCA sample plots with densities before and after batch correction in the sponge data.'}
grid.arrange(sponge.pca.before.plot, sponge.pca.rBE.plot, sponge.pca.ComBat.plot, sponge.pca.PLSDA_batch.plot, sponge.pca.sPLSDA_batch.plot, sponge.pca.PN.plot, ncol = 2)
```
...
We calculate the global explained variance across all microbial variables using pRDA. To achieve this, we create a loop for each variable from the original (uncorrected) and batch-corrected data. The final results are then displayed with `partVar_plot()` from `r colorize("PLSDAbatch", "orange")` package.

```{r Spongeprda, fig.height = 6, fig.align = 'center', fig.cap = 'Global explained variance before and after batch correction for the sponge data.'}
sponge.corrected.list <- list(Before = sponge.clr, 
...
sponge.prop.df <- data.frame(Treatment = NA, Intersection = NA, 
...
The $R^2$ values for each variable are calculated with `lm()` from `r colorize("stats", "orange")` package. To compare the $R^2$ values among variables, we scale the corrected data before $R^2$ calculation. The results are displayed with `plot()` from R `r colorize("base", "orange")` package. We add notes on the top right of the plots to support the interpretation of these particular results.


```{r Sponger2, fig.height = 8, fig.width = 7, out.width = '100%', fig.align = 'center', fig.cap = '$R^2$ values for each microbial variable before and after batch correction in the sponge data.'}
# scale
...
par(mfrow = c(3,2))
for(i in 1:length(sponge.r_values.list)){
  plot(sponge.r_values.list[[i]]$trt, sponge.r_values.list[[i]]$batch, 
     main = names(sponge.r_values.list)[i], xlab = xlabs, ylab = ylabs,
...
lapply(sponge.r_values.list, colSums)[c(2,4,5)]
```

We observed that the data corrected by ComBat, percentile normalisation still included a few variables with a large proportion of batch variance as shown in the above figures. When considering the sum of all variables, the remaining batch variance of corrected data from sPLSDA-batch was greater than PLSDA-batch, which was greater than removeBatchEffect. The preserved treatment variance of corrected data from sPLSDA-batch was greater than PLSDA-batch which itself was greater than removeBatchEffect.
...
```{r Spongealignment, fig.align = 'center', fig.cap = 'Comparison of alignment scores before and after batch correction using different methods for the sponge data.'}
sponge.var = 0.95
sponge.k = 3
...
The alignment scores complement the PCA results, especially when batch effect removal is difficult to assess on PCA sample plots. For example in previous PCA sample plots, we observed that the samples across different batches were better mixed after batch effect correction with different methods than before, whereas the performance of different methods was difficult to compare. Since a higher alignment score indicates that samples are better mixed, as shown in the above figure, sPLSDA-batch gave a superior performance compared to the other methods.
...
```{r SpongeupsetR, out.width = '100%', fig.align = 'center', fig.cap = 'UpSet plot showing overlap between variables selected from different corrected data for sponge study.'}
...
upset(sponge.splsda.upsetR, main.bar.color = 'gray36',
      ...
      queries = list(list(query = intersects, params = list('Before'), 
...
In the above figure, part of the samples with different diets were mixed together, while all the samples from different batches (i.e., cages) were well mixed and not distinct. This result indicated no cage effect for the whole data and no treatment effect for part of samples.

```{r HFHSpcaBefore2, fig.align = 'center', fig.cap = 'The PCA sample plot with densities coloured by days in the HFHS data.'}
...
As shown in the above figure, we did not observe a difference between samples with different diets on arrival day "A" and "Day0". This result was consistent with the experimental design, as mice were not treated with different diets on these two days. We thus removed the samples from these days.
...
In the result, `X1` and `X2` represent the first and second covariates fitted in the model. `[a]`, `[c]` represent the independent proportion of variance explained by `X1` and `X2` respectively, and `[b]` represents the intersection variance shared between `X1` and `X2`. In the `r colorize("HFHS data", "olive")`, the cage and diet effects have a nested batch x treatment design that is extremely unbalanced. We didn't detect any treatment variance (indicated in line `[a]`, Adj.R.squared = 0) but considerable intersection variance (indicated in line `[b]`, Adj.R.squared = 0.271). Thus, all the treatment variance was explained by the intersection variance shared with batch variance. It means batch and treatment effects were collinear. Therefore, the cage effect in the `r colorize("HFHS data", "olive")` could only be accounted for with a linear mixed model, as detailed in section **accounting for batch effects** method "Linear regression".
...
For the day and diet effects, the batch x treatment design was balanced. There was no intersection variance (indicated in line `[b]`, Adj.R.squared = 0). The proportion of treatment variance was much higher than batch variance as indicated in lines `[a]` (Adj.R.squared = 0.272) and `[c]` (Adj.R.squared = 0.036).
...
HFHS.phenotypeData = AnnotatedDataFrame(data =
                                HFHS_data$FullData$metadata[-hfhs.dA0.idx,])
HFHS.taxaData = AnnotatedDataFrame(data = 
                                as.data.frame(HFHS_data$FullData$taxa))
...
```{r HFHSlm, fig.height = 13, fig.width = 12, out.width = '100%', fig.align = 'center', fig.cap = 'Diagnostic plots for the model fitted with the day effect of "OTU 541135" in HFHS data.'}
...
```
To diagnose the validity of the model fitted with both treatment and batch effects, we use different plots to check the assumptions of each microbial variable. For example, the diagnostic plots of "OTU 541135" are shown in the above figure panel. The linearity (or homoscedasticity) and homogeneity of variance were not satisfied (top panel). The correlation between batch (`batch.fix`) and treatment (`trt`) effects was very low, indicating a good model with low collinearity. Some samples could be classified as outliers with a Cook's distance larger than or equal to 0.5, for example, "46", "35", "126", "125" and "16" (middle panel). The distribution of residuals was very close to normal (bottom panel). For the microbial variables with some assumptions not met, we should be careful about their results.
...
```{r HFHSlmm, fig.height = 13, fig.width = 12, out.width = '100%', fig.align = 'center', fig.cap = 'Diagnostic plots for the model fitted with the day and cage effects of "OTU541135" in HFHS data.'}
...
According to the diagnostic plots of "OTU541135" as shown in the above figure panel, the linearity (or homoscedasticity) and homogeneity of variance were not satisfied (top panel). The correlation between batch (`batch.fix`) and treatment (`trt`) effects was very low, indicating a good model with low collinearity. Some samples could be classified as outliers with a Cook's distance larger than or equal to 0.5, for example, "16", "35", "126", "125" and "136" (middle panel). The distribution of residuals and random effects was very close to normal (bottom panel). 
...
The methods that we use to correct for batch effects include removeBatchEffect, ComBat, PLSDA-batch, sPLSDA-batch, Percentile Normalisation and RUVIII. Among them, RUVIII was designed for unknown batch effects. 

Since the cage effect cannot be corrected for because of the collinearity between the cage and treatment effects and the cage effect only accounts for a small amount of the total data variance (1.3% calculated with pRDA), so we only correct for the day effect in the `r colorize("HFHS data", "olive")`.

**removeBatchEffect**

The `removeBatchEffect()` function is implemented in `r colorize("limma", "orange")` package. The design matrix (`design`) with treatment grouping information can be generated with `model.matrix()` function from `r colorize("stats", "orange")` as shown in section "SVA".
...
Here the optimal number of variables to select for the treatment component is 2. Since we adjust the amount of treatment variation to preserve, we need to re-choose the optimal number of components related to batch effects using the same criterion mentioned in section "PLSDA-batch". 
...
```{r HFHSpca, fig.height = 17, fig.width = 12, out.width = '100%', echo = F, fig.align = 'center', fig.cap = 'The PCA sample plots with densities before and after batch correction in HFHS data.'}
grid.arrange(hfhs.pca.before.plot, hfhs.pca.rBE.plot, hfhs.pca.ComBat.plot, hfhs.pca.PLSDA_batch.plot, hfhs.pca.sPLSDA_batch.plot, hfhs.pca.PN.plot, hfhs.pca.RUVIII.plot, ncol = 2)
```

In Figure \@ref(fig:HFHSpca), the differences between the samples from different days are well removed after batch correction with PLSDA-batch and RUVIII. We can also compare the boxplots and density plots for key variables identified in PCA driving the major variance or in heatmaps showing obvious patterns before and after batch effect correction (results not shown).
...
```{r HFHSprda, fig.height = 6, fig.align = 'center', fig.cap = 'Global explained variance before and after batch correction for HFHS data.'}
# HFHS data
hfhs.corrected.list <- list(Before = hfhs.clr.less, 
...
```{r HFHSr2, fig.height = 8, fig.width = 7, out.width = '100%', fig.align = 'center', fig.cap = '$R^2$ values for each microbial variable before and after batch correction in HFHS data.'}
# scale
...
symbol_vector <- c(expression('bad'), expression('good'), 
                   ...
                   expression('bad'), expression('bad'),
                   expression('good'))
symbol_color <- c('red', 'dark green', 
...
par(mfrow = c(4,2))
for(i in 1:length(hfhs.r_values.list)){
  plot(hfhs.r_values.list[[i]]$trt, 
       hfhs.r_values.list[[i]]$batch, 
       main = names(hfhs.r_values.list)[i], 
...
# the sum of all variables
lapply(hfhs.r_values.list, colSums)[c(2,3,4,7)]
```

We observed from these plots that the data corrected by sPLSDA-batch, percentile normalisation still included many variables with a large proportion of batch variance as shown in the above figure panel. When considering the sum of all variables, the remaining batch variance of corrected data from PLSDA-batch is greater than removeBatchEffect, ComBat and RUVIII. The preserved treatment variance of corrected data from PLSDA-batch is the largest.
...
```{r HFHSalignment, fig.align = 'center', fig.cap = 'Comparison of alignment scores before and after batch correction using different methods for the HFHS data.'}
# HFHS data
hfhs.var = 0.95
hfhs.k = 15
...
The alignment scores complement the PCA results, especially when batch effect removal is difficult to assess on PCA sample plots. For example in the PCA plots for the `r colorize("HFHS data", "olive")`, we observe that the samples across different batches were better mixed after batch correction with PLSDA-batch and RUVIII than before, while the performance of these two methods were difficult to compare. Since a higher alignment score indicates that samples are better mixed, as shown in the above figure, RUVIII gave a superior performance compared to the other methods.


We recommend the sparse version of PLSDA-batch for a very strong batch effect, for example, in the `r colorize("AD data", "blue")`. However, for a weak batch effect, we do not recommend removing batch effects in `r colorize("HFHS data", "olive")`, but if we have to, we recommend using PLSDA-batch. It is easier to lose treatment variation when the batch variation is very small. Therefore PLSDA-batch can better ensure the complete preservation of treatment variation than a sparse version. 

**Variable selection**

We use `splsda()` from `r colorize("mixOmics", "orange")` to select the top 50 microbial variables that, in combination, discriminate the different treatment groups in the `r colorize("sponge data", "brown")`. We apply `splsda()` for the different batch corrected data from all methods. Then we use `upset()` from `r colorize("UpSetR", "orange")` package [@lex2014upset] to visualise the concordance of variables selected. 

In the code below, we first need to convert the list of variables selected from different method-corrected data into a data frame compatible with `upset()` using `fromList()`. We then assign different colour schemes for each variable selection.


```{r, HFHSupsetR, out.width = '100%', fig.align = 'center', fig.cap = 'UpSet plot showing overlap between variables selected from different corrected data for sponge study.'}
...
In Figure \@ref(fig:HFHSupsetR), the left bars indicate the number of variables selected from each data corrected with different methods. The right bar plot combined with the scatterplot shows different intersection and their aggregates. We obtain an overlap of 39 out of 50 selected variables between different corrected and uncorrected data. However, the data from each method still include unique variables that are not selected in the other corrected data. As `upset()` can only include five datasets at once, we only display the uncorrected data and four corrected data that have been efficiently corrected for batch effects from our previous assessments.
...
The raw `r colorize("HD data", "violet")` include 368 OTUs and 30 samples. We use the function `PreFL()` from our `r colorize("PLSDAbatch", "orange")` R package to filter the data. After filtering, the `r colorize("HD data", "violet")` are reduced to 269 OTUs and 30 samples. The proportion of zeroes is reduced from 45% to 33%.
...
```{r HDpcaBefore, fig.align = 'center', fig.cap = 'The PCA sample plot with densities in HD data.'}
# HD data
...
```{r, HDheatmap, out.width = '90%', fig.align = 'center', fig.cap = 'Hierarchical clustering for samples in HD data.'}
# scale the clr data on both OTUs and samples
...
In the result, `X1` and `X2` represent the first and second covariates fitted in the model. `[a]`, `[c]` represent the independent proportion of variance explained by `X1` and `X2` respectively, and `[b]` represents the intersection variance shared between `X1` and `X2`. Collinearity was detected in the `r colorize("HD data", "violet")` between treatment and batch as indicated by lines `[a]` and `[b]` that all treatment variance (`[a]`: Adj.R.squared = 0, `[b]`: Adj.R.squared = 0.0973) was explained as intersection variance. The batch x treatment design is nested, in such a design the intersection variance is usually considerable. As the intersection is shared between batch and treatment effects, the batch variance in the `r colorize("HD data", "violet")` is larger than the treatment variance.
...
We apply a `linear mixed model` and fit the batch effect as a random effect because the design of `r colorize("HD data", "violet")` is nested. In such design, the number of batches should be larger than treatment groups; otherwise, the linear mixed model cannot address the batch effect. Note: we assume that there is no interaction between batch and treatment effects on the microbial variable. 
...
```{r HDlmmS, fig.height = 13, fig.width = 12, out.width = '100%', fig.align = 'center', fig.cap = 'Diagnostic plots for the model fitted with the cage effect of "OTU1" in HD data.'}
...
According to the diagnostic plots of "OTU1" as shown in the above figure panel, the linearity (or homoscedasticity) and homogeneity of variance were not satisfied (top panel). Some samples could be classified as outliers with a Cook's distance larger than or equal to 0.5, for example, "12", "21", "20", "11" and "6" (middle panel). The distribution of residuals was not normal, while the one of random effects was very close to normal (bottom panel). 
...
Since we apply a `"linear mixed model"` to the `r colorize("HD data", "violet")`, this type of model can only output conditional $R^2$ that includes the variance of both fixed and random effects (treatment, fixed and random batch effects) and marginal $R^2$ that includes only the variance of fixed effects (treatment and fixed batch effects) [@nakagawa2013general]. The marginal $R^2$s of the models with and without the batch effect should be the same as marginal $R^2$ only includes fixed effects and the treatment effect is the only fixed effect here. The results of these two models are not the same due to the actual calculation.
